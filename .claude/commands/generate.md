...
d.update({
  ...
  'next_action': 'start_evaluator_session',
  'artifacts': {**d.get('artifacts', {}), 'handoff': '.worktrees/{title}/handoffs/{title}.md'},
})
if prev and prev != 'handoff_ready':
...
- 컨텍스트 덤프 조건(태스크 5개 완료 / 파일 누적 10개 초과 / REVIEW 후 잔여 태스크 3개 이상)에 해당하면 세션 분할 — 재량 판단 금지, 조건 도달 즉시 분할
